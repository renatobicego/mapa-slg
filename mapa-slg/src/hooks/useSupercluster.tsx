...
export function useSupercluster<T extends GeoJsonProperties>(
  ...
  // Helper function to get images from a specific cluster
  const getClusterAvatars = useCallback(
    (clusterId: number, limit = 3) => {
      const leaves = clusterer.getLeaves(clusterId, limit);
      return leaves;
    },
    [clusterer]
  );
